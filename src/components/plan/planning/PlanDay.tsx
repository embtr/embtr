                ...
                taskViews.push(
                    <View key={plannedTask.routine.id + '_locked'} style={{ paddingBottom: 5, alignItems: 'center' }}>
                        <PlanningTask
                            ...
                            locked={locked === true}
                            isChecked={updatedPlannedTasks.get(plannedTask.routine.id ? plannedTask.routine.id : plannedTask.id!)?.checked !== false}
                            onCheckboxToggled={onChecked}
                            onUpdate={onPlannedTaskUpdate}
                            goal={taskGoal}
                            pillar={taskPillar}
                        />
                    </View>
    ...
    const getUpdatedPlannedDay = (): PlannedDay => {
        ...
        plannedDay?.plannedTasks.forEach((plannedTask) => {
            ...
            if (updatedPlannedTask) {
                ...
                if (taskIsEnabled) {
                    const newPlannedTask: PlannedTaskModel = createPlannedTaskByPlannedTask(
                        plannedTask,
                        updatedPlannedTask.startTime,
                        updatedPlannedTask.duration
                    );
